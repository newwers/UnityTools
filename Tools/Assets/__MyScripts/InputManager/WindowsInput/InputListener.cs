...
public static class InputListener
{
    ...
    const int WH_MOUSE_LL = 14;//全局鼠标消息id
    ...
    public static event Action OnAnyInputEvent;
    public static event Action OnMouseInputEvent;
    public static event Action OnKeyboardInputEvent;
    public static event Action<KeyCode> OnKeyDownInputEvent;
    public static event Action<KeyCode> OnKeyUpInputEvent;
    ...
    public static void CreateMouseHook()
    {
    ...
    public static void CreateKeyboardHook()
    {
    ...
    static IntPtr KeyboardCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        if (nCode >= 0)
        {
            ...
            if (VKCodeToKeyCode.ContainsKey(code))
            {
                ...
                {
                    if (down != null && down.Contains(keyCode))
                    {
                        KeyCallback(keyCode, hold);//键盘按住
                    }
                    else if (hold != null && !hold.Contains(keyCode))//键盘按下
                    {
                        KeyCallback(keyCode, down, onKeyDown);

                        OnKeyDownInputEvent?.Invoke(keyCode);
                    }
                }
                else if (wParam == (IntPtr)WM_KEYUP)//键盘松开
                {
                    KeyCallback(keyCode, up, onKeyUp);
                    hold?.Remove(keyCode);
                    OnKeyUpInputEvent?.Invoke(keyCode);
                }

            }
            else UnityEngine.Debug.LogWarning("[Desktopia/Inputs] VKCode " + code + " doesn't exist in database, try adding it!");

            if (OnKeyboardInputEvent != null)
            {
                OnKeyboardInputEvent.Invoke();
            }

            if (OnAnyInputEvent != null)
            {
                OnAnyInputEvent.Invoke();
            }
        }
    ...
    static IntPtr MouseCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        if (nCode >= 0)
        {
            ...
            {
                //if (code == WM_MOUSEMOVE)//鼠标移动
                //{
                //    for (int i = 0; i < onMouseMove.Count; i++)
                //    {
                //        onMouseMove[i].Invoke();//todo:不知道在哪移除
                //    }
                //    CallNextHookEx(mouseHook, nCode, wParam, lParam);//鼠标移动
                //}
    ...
    {
        return hold.Count > 0  || mouseHold.Count > 0 || mouseDown.Count > 0 || mouseUp.Count > 0;
    }
