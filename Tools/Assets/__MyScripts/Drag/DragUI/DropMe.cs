...
public class DropMe : MonoBehaviour, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
  ...
  //      }
	...
	public void DropByDragImage(RectTransform rectTransform)
	{
		rectTransform.position = receivingImage.transform.position;
		//rectTransform.SetParent(receivingImage.transform);
    }
	...
	public void OnPointerEnter(PointerEventData data)
	{
		...
		//根据拖拽图片跟容器层级索引进行自动排序,保证拖拽图片显示在最前面
		int siblingindexDrag = data.pointerDrag.transform.GetSiblingIndex();
		int siblingindexDrop = containerImage.transform.GetSiblingIndex();

		if (siblingindexDrag < siblingindexDrop)
		{
			data.pointerDrag.transform.SetSiblingIndex(siblingindexDrop);
		}

		//Sprite dropSprite = GetDropSprite(data);//获取鼠标点到的容器的sprite
		//if (dropSprite != null)//判断是否被拖拽
		//{

		//      }

		containerImage.color = highlightColor;//设置容器的颜色为高亮色
