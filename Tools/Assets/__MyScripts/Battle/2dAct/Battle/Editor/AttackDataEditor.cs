...
{
    private Dictionary<GameObject, AnimationClip> previewAnimations = new Dictionary<GameObject, AnimationClip>();

    public void RegisterPreview(GameObject obj, AnimationClip clip)
    {
        previewAnimations[obj] = clip;
    }

    public void UnregisterPreview(GameObject obj)
    {
        previewAnimations.Remove(obj);
    }

    public void UpdateAllPreviews(float deltaTime)
    {
        foreach (var kvp in previewAnimations)
        {
...
public class AttackDataEditor : EditorWindow
{
    ...
    private EditorPreviewHelper previewHelper;
    ...
    // 所有可编辑的攻击数据列表
    private List<ActionData> allAttacks = new List<ActionData>();
    ...
    {
        GetWindow<AttackDataEditor>("攻击编辑器");
    }
    ...
    private void OnEnable()
    {
        ...
        // 初始化预览系统
        previewHelper = new EditorPreviewHelper();
    ...
    {
        if (!showHitboxPreview || attackManager == null) return;
    ...
    private void LoadLastPreviewCharacter()
    {
        if (attackManager)
        {
            SetPreviewCharacter(attackManager.character.gameObject);
        }
    }
    ...
    private void SetPreviewCharacter(GameObject characterPrefab)
    {
        ...
        if (previewCharacter != null)
        {
            ...
            if (!string.IsNullOrEmpty(path))
            {
                ...
                if (character && attackManager.character != character)
                {
                    attackManager.character = character;
                }
            }
        }
    ...
    private void DrawToolbar()
    {
        ...
        {
            string path = EditorUtility.OpenFilePanel("选择AttackManager", "Assets", "asset");
            if (!string.IsNullOrEmpty(path))
            {
                path = "Assets" + path.Substring(Application.dataPath.Length);
                attackManager = AssetDatabase.LoadAssetAtPath<ActionManager>(path);
                // 保存路径到EditorPrefs
        ...
        if (attackManager != null)
        {
            ...
            {
                EditorUtility.SetDirty(attackManager);
                // 标记所有攻击数据为脏
    ...
    // 加载项目中所有AttackData的方法
    private void LoadAllAttackDataInProject()
    {
        allAttacks.Clear();
        ...
        foreach (string guid in guids)
        {
            ...
            ActionData attackData = AssetDatabase.LoadAssetAtPath<ActionData>(path);
            if (attackData != null)
            {
        ...
        Debug.Log($"找到 {allAttacks.Count} 个AttackData资源");
    ...
    private void LoadLastOpenFile()
    {
        if (EditorPrefs.HasKey(LastOpenPathKey))
        {
            ...
            {
                attackManager = AssetDatabase.LoadAssetAtPath<ActionManager>(lastPath);
            }
            else
            {
                // 如果文件不存在，清除无效的路径记录
    ...
    {
        EditorGUILayout.HelpBox("请加载或创建一个AttackManager来开始编辑攻击数据", MessageType.Info);

        if (GUILayout.Button("创建新的AttackManager", GUILayout.Height(40)))
        {
            CreateNewAttackManager();
        }
    }
    ...
    private void DrawAttackManager()
    {
        ...
        ActionManager newAttackManager = (ActionManager)EditorGUILayout.ObjectField("攻击管理器", attackManager, typeof(ActionManager), false);
        if (newAttackManager != attackManager)
        {
            attackManager = newAttackManager;
            if (attackManager != null)
            {
                string path = AssetDatabase.GetAssetPath(attackManager);
                EditorPrefs.SetString(LastOpenPathKey, path);
            }
            else
            {
        ...
        if (attackManager.normalAttacks == null)
            attackManager.normalAttacks = new ActionData[0];

        // 普通攻击序列编辑
        EditorGUILayout.Space();
        EditorGUILayout.LabelField("普通攻击序列", EditorStyles.boldLabel);

        int newCount = EditorGUILayout.IntField("攻击数量", attackManager.normalAttacks.Length);
        if (newCount != attackManager.normalAttacks.Length)
        {
            System.Array.Resize(ref attackManager.normalAttacks, newCount);
        }

        for (int i = 0; i < attackManager.normalAttacks.Length; i++)
        {
            EditorGUILayout.BeginHorizontal();
            attackManager.normalAttacks[i] = (ActionData)EditorGUILayout.ObjectField(
                $"攻击 {i + 1}", attackManager.normalAttacks[i], typeof(ActionData), false);

            // 添加跳转按钮
            if (attackManager.normalAttacks[i] != null)
            {
                if (GUILayout.Button("跳转", GUILayout.Width(50)))
                {
                    Selection.activeObject = attackManager.normalAttacks[i];
                    EditorGUIUtility.PingObject(attackManager.normalAttacks[i]);
                }
            }
            EditorGUILayout.EndHorizontal();
        }
        ...
        DrawSpecialAttackField("冲刺攻击", ref attackManager.dashAttack);
        DrawSpecialAttackField("跳跃攻击", ref attackManager.jumpAttack);
        DrawSpecialAttackField("特殊攻击", ref attackManager.specialAttack);
        DrawSpecialAttackField("重攻击", ref attackManager.heavyAttack);
        DrawSpecialAttackField("弹反攻击", ref attackManager.parryAttack);
    }

    private void DrawSpecialAttackField(string label, ref ActionData attackData)
    {
        EditorGUILayout.BeginHorizontal();
        attackData = (ActionData)EditorGUILayout.ObjectField(label, attackData, typeof(ActionData), false);
    ...
    private void DrawAttackEditor()
    {
        ...
        // 加载所有AttackData按钮
        if (GUILayout.Button("加载所有AttackData", EditorStyles.miniButtonMid))
        {
            LoadAllAttackDataInProject();
        }
    ...
    private void DrawAttackSelection()
    {
        ...
        if (allAttacks.Count > 0)
        {
            ...
            if (newSelectedIndex != selectedAttackIndex)
            {
                ...
                // 尝试找到此AttackData所属的AttackManager
        ...
        {
            EditorGUILayout.HelpBox("没有找到AttackData资源，点击工具栏上的'加载所有AttackData'按钮", MessageType.Info);
        }
    ...
    // 查找包含选定AttackData的AttackManager
    private void FindAttackManagerForSelectedAttack(ActionData attackData)
    {
        if (attackManager != null && attackManager.normalAttacks != null)
        {
            // 检查当前AttackManager是否包含此AttackData
            if (Array.IndexOf(attackManager.normalAttacks, attackData) >= 0 ||
                attackManager.dashAttack == attackData ||
                attackManager.jumpAttack == attackData ||
                attackManager.specialAttack == attackData ||
                attackManager.heavyAttack == attackData)
            {
                // 已经在当前AttackManager中
                return;
            }
        ...
        // 查找所有AttackManager资源
        string[] guids = AssetDatabase.FindAssets("t:AttackManager");
        foreach (string guid in guids)
        {
            string path = AssetDatabase.GUIDToAssetPath(guid);
            ...
            if (manager != null)
            {
                // 检查此AttackManager是否包含选定的AttackData
                    ...
                    manager.heavyAttack == attackData)
                {
                    ...
                    Debug.Log($"找到包含此AttackData的AttackManager: {manager.name}");
                    return;
                }
            }
        }

        Debug.Log("未找到包含此AttackData的AttackManager");
    }


    private void DrawAttackDetails(ActionData attack)
    {
        // 设置当前预览动画
        ...
        attack.animationParameterType = (ActionData.AnimationParameterType)EditorGUILayout.EnumPopup("参数类型", attack.animationParameterType);
        attack.animationParameterName = EditorGUILayout.TextField("参数名称", attack.animationParameterName);

        // 根据选择的参数类型显示不同的值字段
        switch (attack.animationParameterType)
        {
            case ActionData.AnimationParameterType.Int:
                attack.animationIntValue = EditorGUILayout.IntField("整数值", attack.animationIntValue);
                break;
            case ActionData.AnimationParameterType.Float:
                attack.animationFloatValue = EditorGUILayout.FloatField("浮点值", attack.animationFloatValue);
                break;
            case ActionData.AnimationParameterType.Bool:
                attack.animationBoolValue = EditorGUILayout.Toggle("布尔值", attack.animationBoolValue);
                break;
    ...
    // 同步到预览帧
    private void SyncToPreviewFrame(ActionData attack)
    {
    ...
    // 在预览帧创建新帧数据
    private void CreateFrameAtPreviewFrame(ActionData attack)
    {
        ...
        newFrame.hitboxRadius = attack.hitboxRadius;
        newFrame.hitboxAngle = attack.hitboxAngle;
        newFrame.hitboxEndPoint = attack.hitboxEndPoint;
        newFrame.damage = 1;
    ...
    // 添加创建新帧数据的方法
    private void CreateNewFrameData(ActionData attack)
    {
        AttackFrameData newFrame = new AttackFrameData();
    ...
    private void DrawFrameData(AttackFrameData frameData, int index, ActionData attack)
    {
        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
    ...
    private void SyncToAnimationLength(ActionData attack)
    {
        if (attack.animationClip != null)
        {
    ...
    private void DrawTimelinePreview(ActionData attack)
    {
        EditorGUILayout.Space();
    ...
    private void CreateNewAttackManager()
    {
        ...
        string path = EditorUtility.SaveFilePanel("保存AttackManager", "Assets", "NewAttackManager", "asset");
        if (!string.IsNullOrEmpty(path))
        {
            path = "Assets" + path.Substring(Application.dataPath.Length);
            AssetDatabase.CreateAsset(newManager, path);
            AssetDatabase.SaveAssets();
            attackManager = newManager;
