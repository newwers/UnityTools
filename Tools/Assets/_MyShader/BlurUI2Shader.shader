Shader "Custom/BlurUI2Shader" {
    Properties {
         _MainTex ("Sprite Texture", 2D) = "white" {}//Image组件上的Source Image
    ...
    SubShader {
        ...
        Pass {//先绘制背景,用 GrabPass 进行抓取(消耗高),
            CGPROGRAM
            #pragma vertex vert
            ...
            half4 frag (v2f i) : SV_Target {
                ...
                for (float x = -_BlurRadius; x <= _BlurRadius; x += 1) {// 在x方向上进行模糊计算
                    for (float y = -_BlurRadius; y <= _BlurRadius; y += 1) {// 在y方向上进行模糊计算
                        ...
                        blurCol += tex2D(_GrabTexture, i.uv + offset);// 累加模糊颜色
                    }
                }
